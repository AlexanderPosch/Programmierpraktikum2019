import sys
sys.path.append('D:\Python\site-packages')
import sympy as sy
print("Import done")
...
V=g*sum(map(lambda k: m[k]*sum(map(lambda i:-l[i]*sy.cos(y[i]),list(range(k+1)))),list(range(n+1))))
def ugly1(k):
    return sum(map(lambda i: l[i]*v[i]*sy.cos(y[i]),list(range(k+1))))
def ugly2(k):
    return sum(map(lambda i: l[i]*v[i]*sy.sin(y[i]),list(range(k+1))))
T=1/2*sum(map(lambda k: m[k]*(ugly1(k)**2+ugly2(k)**2),list(range(n+1))))
L=T-V 
...
Tdv=Tdv.subs(list(map(lambda k:(v[k],y[k](t).diff(t)),range(n+1)))) #v=y'(t)
Tdv=Tdv.subs(list(map(lambda k:(a[k],y[k](t).diff(t,t)),range(n+1)))) #a=y''(t)
Tdv=Tdv.subs(list(map(lambda k:(y[k],y[k](t)),range(n+1)))) #y=y(t)

#Differentiate Tdv by t
Tdvdt=sy.Matrix([*map(lambda x:x.diff(t),Tdv)])
print(Tdvdt)
Lagrangev=Tdvdt-Ldy #This is Matrix holds a list of n+1 linear equations in a0,a1,a2,... (here still in the form of second derivatives)

#Substituting Derivatives with Variables for clarity
Lagrangev=Lagrangev.subs(list(map(lambda k:(y[k](t).diff(t,t),a[k]),range(n+1)))) #y''(t)=a   
Lagrangev=Lagrangev.subs(list(map(lambda k:(y[k](t).diff(t),v[k]),range(n+1)))) #y'(t)=v
Lagrangev=Lagrangev.subs(list(map(lambda k:(y[k](t),y[k]),range(n+1)))) #y(t)=y
